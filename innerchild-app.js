(function () {
  ...
  function drawReading(count) {
    return shuffleArray(CHILD_DECK).slice(0, count);
  }
  ...
  function showDetail(card) {
    cardDetail.innerHTML = `
      ...
      <div class="detail-block essence">
        <div class="detail-label essence">創傷本質</div>
        <p>${card.essence}</p>
      </div>
      <div class="detail-block heal">
        <div class="detail-label heal">溫暖療癒句</div>
        <p>「${card.heal}」</p>
      </div>
    `;
  ...
  revealAllBtn.addEventListener("click", () => {
    document.querySelectorAll(".child-card").forEach((el) => el.classList.add("revealed"));
  });
...
(function () {
  ...
  fetch("https://api.countapi.xyz/hit/maureenyang-tarot-select/innerchild")
    .then((res) => res.json())
    .then((data) => {
