(function () {
  ...
  function drawReading(count) {
    return shuffleArray(ANGEL_DECK).slice(0, count);
  }

  function buildCardElement(card, positionLabel) {
    const wrapper = document.createElement("div");
    wrapper.className = "angel-card";
  ...
  function showDetail(card) {
    cardDetail.innerHTML = `
      ...
      <div class="guidance-label">落地指引</div>
      <p>${card.guidance}</p>
    `;
  ...
  function buildResultText(cards) {
    ...
    cards.forEach((card, i) => {
      const label = cards.length === 1 ? "指引" : `第 ${i + 1} 張`;
      lines.push(`${label}：${card.name}｜${card.en}`);
      lines.push(card.guidance);
      lines.push("");
    });
  ...
  revealAllBtn.addEventListener("click", () => {
    document.querySelectorAll(".angel-card").forEach((el) => el.classList.add("revealed"));
  });
...
(function () {
  ...
  fetch("https://api.countapi.xyz/hit/maureenyang-tarot-select/angel")
    .then((res) => res.json())
    .then((data) => {
